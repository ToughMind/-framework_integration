package lq.common.dao.hibernate;
 ...
 */
public abstract class BaseDaoImpl<M extends Serializable, PK extends Serializable> extends HibernateDaoSupport
	...
	public <T> List<T> listByNative(final String nativeSQL, final int pn, final int pageSize,
		...
		List<T> result = getHibernateTemplate().execute(new HibernateCallback<List<T>>() {
	...
	@SuppressWarnings("unchecked")
	protected <T> T uniqueByNative(final String natvieSQL, final List<Entry<String, Type>> scalarList,
			final Object... paramlist) {
	...
	// @SuppressWarnings("unchecked")
	// public <T> List<T> list(ConditionQuery query, OrderBy orderBy, final int
	// pn, final int pageSize) {
